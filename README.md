...
3. **覆蓋檔案**：
   * 打開您的 Minecraft 遊戲根目錄。
   * 將解壓出的 `config` 資料夾直接拖入遊戲根目錄。
   * 系統提示是否取代現有檔案時，請選擇 **「是」**。
4. **立即生效**：
   * 若遊戲已啟動，請在對話框輸入 `/ftbquests reload` 並送出。
   * 或重新啟動遊戲。

---

### 🛠️ 方案 C：完整覆蓋安裝 (Overrides)
此方式適用於整合包製作者，或希望將翻譯直接整合進遊戲檔案的玩家。

1. **下載檔案**：下載 `ATM10_Full_Patch_vX.X.X.zip`。
2. **備份原始檔案**：建議先備份您遊戲目錄下的 `overrides` 或 `config` 資料夾。
3. **解壓覆蓋**：將 Zip 內的內容全部解壓至遊戲根目錄，並覆蓋所有重複檔案。

---

### ✅ 驗證安裝是否成功
進入遊戲後，您可以檢查以下內容：
* **物品名稱**：檢查 `Modern Industrialization` 的機器或 `Allthemodium` 的礦石是否顯示為繁體中文。
* **任務書**：按下快捷鍵 (預設為 `Quest` 按鈕) 檢查章節標題與描述是否已中文化。
* **系統提示**：嘗試使用被禁用的物品，觀察狀態列是否出現繁體中文警告。
